...
```dart
final dipose = effect(() {
  print(counter.value); // 最初に一回、その後はcounterの値が更新される度にprintが呼ばれる
});
...
今回の場合、例えばStreamの`listen`だとcounterの値が増える度に1、2、3…と変更が通知されますが、`effect`の場合0、1、2…となります。
...
final dispose = counter.subscribe((value) {
  print(value); // 最初に一回、その後はcounterの値が更新される度にprintが呼ばれる
});
...
build(BuildContext context) {
  ...
  counter.subscribe((value) {
    // counterの値が一度更新されるだけでprintが何回も呼ばれてしまう
    print(value);
  });
...
build(BuildContext context) {
  // このWidgetが再描画されてlistenが再度呼ばれても一回分のみ通知を受け取る
...
* `runApp`の直下に管理用Widgetを置かなくても良いので、他の状態管理パッケージと併用できる
  * アプリ全体の状態管理はRiverpodを使用し、`StatefulWidget`内で完結する場合は`setState`の代わりに`createSignal`を使うなど
