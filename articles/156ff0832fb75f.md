...
str!.contains("dog");
// 以降、同じ範囲（スコープ）であればStringとして扱えるため、! を外せる
...
ただしグローバル変数のように様々な場所で代入できる変数の場合、if文で確認してから使うまでの間にnullが入る可能性があるので `!`を外せません。
...
あまり使い道の無さそうな書き方ですが、「重い処理を一度だけ行える」ので、アプリの動作の高速化などに使えます。
